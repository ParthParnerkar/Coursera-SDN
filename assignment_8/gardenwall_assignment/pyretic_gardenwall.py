...
class pyretic_gardenwall(DynamicPolicy):
    ...
    def update_policy(self, flow):
        ...
        for f in self.flow_state_map:
            ...
            srcip_field  = f.get('srcip')

            if srcmac_field is not None or srcip_field is not None:
                this_policy = None
                ...
                if infected_state == "True" and exempt_state == "True":
                    if srcmac_field is not None:
                        this_policy = (match(srcmac=EthAddr(srcmac_field)) >> modify(dstip=IPAddr('10.0.0.3')) >> 
                                       modify(dstmac=EthAddr("00:00:00:00:00:03")) >> flood()) + (~match(srcmac=EthAddr(srcmac_field)) 
                                       >> flood())
                    if srcip_field is not None:
                         this_policy = (match(srcmac=IPAddr(srcip_field)) >> modify(dstip=IPAddr('10.0.0.3')) >> 
                                       modify(dstmac=EthAddr("00:00:00:00:00:03")) >> flood()) + (~match(srcip=IPAddr(srcip_field)) 
                                       >> flood())
