...
-}
mtch_size_new = tbl_size_1
mtch_tbl_new = Dynamic("mtch0_new", (mtch_size_new, ["srcmac","dstmac"]))
mtch_val_new  = Static([[("srcmac", 0),("dstmac",0)] | x <- [1..mtch_size_new]])
mtch_ptrn_new = [("srcmac", "dstmac")] 
...
-}
	--compare the header with ACL table
	,BRTF(mtch_tbl_new, "i", "drp")
    --Drop the tag header   
    ,LBL("drp")
	,DRP
